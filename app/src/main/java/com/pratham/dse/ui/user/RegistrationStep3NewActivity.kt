...
import kotlinx.android.synthetic.main.activity_registration_step3.*
import java.io.File
...
class RegistrationStep3NewActivity : BaseActivity() {
    private var mImageFilePath: String? = null

    companion object {
        fun startActivityForResult(context: Activity, bundle: Bundle?, flag: Int) {
            val intent = Intent(context, RegistrationStep3NewActivity::class.java)
            bundle?.let {
    ...
    private fun initUI() {
        ...
        tvNext.setOnClickListener {
            if (TextUtils.isEmpty(mImageFilePath)) {
                Toast.makeText(
                    applicationContext,
                    getString(R.string.msg_capture_aadhar_image),
                    Toast.LENGTH_LONG
                ).show()
            } else {
                ...
                userRegistrationModel.let {
                    it.aadhaarImageName = File(mImageFilePath).name
                    it.aadhaarImagePath = mImageFilePath
                    uploadAadhaarPhoto(it)
    ...
    private fun checkAndOpenCamera() {
        if (Utils.checkPermission(
                this@RegistrationStep3NewActivity,
            ...
            if (Utils.checkPermission(
                ...
                CropImage.activity()
                    ...
                    .start(this@RegistrationStep3NewActivity);
            } else {
                requestPermission(
                    Manifest.permission.CAMERA,
    ...
    private fun uploadAadhaarPhoto(userRegistrationModel: UserRegistrationModel) {
        val mAviLoader = TransAviLoader(mBaseActivity, true)
        mAviLoader.setLoaderColor(ContextCompat.getColor(mBaseActivity, R.color.matColor8))
        mAviLoader.show()
        val base64Str = ImageUtil.getFileToBase64(userRegistrationModel.aadhaarImagePath)
        RegistrationApiProvider(mApplication).uploadDocument(
            ...
            object :
                ...
                override fun onApiFailed(errorMsg: String) {
                    Log.e(RegistrationStep1Activity.TAG, "uploadAadhaarPhoto() $errorMsg")
                    if (mAviLoader.isShowing) {
                        mAviLoader.dismiss()
                    }
