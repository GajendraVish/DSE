package com.pratham.dse.ui.Techs.conveyance.without_jammer
...
class ConvenyanceLogWithoutJammerActivity : BaseActivity() {

    private var mLocation: Location? = null
    private var mediums: Array<String>? = null

    companion object {
        private val TAG = ConvenyanceLogWithoutJammerActivity::class.java.simpleName
        fun startActivity(context: Context, bundle: Bundle?) {
            val intent = Intent(context, ConvenyanceLogWithoutJammerActivity::class.java)
            bundle?.let {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        setContentView(R.layout.activity_conveyance_log_without_jammer)
    ...
    private fun initUI() {
        supportActionBar?.title = getString(R.string.title_conveyance_log)
    ...
    private fun requestPermission(permission: String, requestCode: Int) {
        ActivityCompat.requestPermissions(
            this@ConvenyanceLogWithoutJammerActivity,
            arrayOf(permission),
            requestCode
        )
    }
...
//            if (SmartLocation.with(mBaseActivity).location().state().isGpsAvailable) {
            SmartLocation.with(this@ConvenyanceLogWithoutJammerActivity).location()
                .oneFix()
                .start {
                    mLocation = it
                }
